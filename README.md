...
当前发布版本为 **0.1.4**。可从 0.1.3 覆盖安装升级，安装到默认的
`C:\Program Files\Lens Cycle` 不会覆盖用户数据库或 JSON 备份。本次没有 schema 变更，
继续使用 0.1.2 已升级的数据格式。

0.1.4 主要更新：

- 支持删除已经恢复或结束的历史暂停片段，并自动合并相邻使用阶段；
- 修改开封瓶产品的开封后有效期时，原子同步关联时间轴实例的开封失效日和最终预测日；
- 时间轴按产品顺序分配连续轨道，同一产品实例保持上下相邻。

升级前仍建议从设置页创建一次 JSON 备份。实现、迁移和验收记录见
[0.1.4 发布记录](./docs/release_0.1.4.md)及 [测试报告](./docs/test_report.md)。
