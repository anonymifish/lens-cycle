...
当前发布版本为 **0.1.2**。从 0.1.1 覆盖安装即可升级，安装到默认的
`C:\Program Files\Lens Cycle` 不会覆盖用户数据库或 JSON 备份；数据库在首次由 0.1.2
打开时通过新增迁移自动升级，已发布的初始迁移不会被改写。

0.1.2 主要更新：

- 正常退出和重启会等待待提交保存，执行 `PRAGMA wal_checkpoint(TRUNCATE)`，显式关闭
  SQLite 后才结束进程；收尾失败时应用保持打开并显示具体错误；
- 平均单位价格改用 `1 元 = 10000` 的定点整数存储，支持输入、编辑、备份恢复和固定显示
  4 位小数；库存价值与购买支出最终按人民币金额规则舍入；
- 兼容 0.1.1 数据库和旧版两位小数备份。旧版本已经丢失的第 3、4 位不会被猜测恢复，
  升级后可在库存批次中手动更正。

升级前仍建议从设置页创建一次 JSON 备份。实现、迁移和验收记录见
[0.1.2 发布记录](./docs/release_0.1.2.md)及 [测试报告](./docs/test_report.md)。
