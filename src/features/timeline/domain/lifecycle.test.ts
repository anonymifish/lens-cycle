...
const item: TimelineItem = {
  ...
  location: "家",
  locationId: "home",
  locationIntervals: [{ locationId: "home", startDate: "2026-07-01", endDate: null }],
  eyeSides: ["L"],
  eyeAssignmentIntervals: [{ eyeSides: ["L"], startDate: "2026-07-01", endDate: null }],
...
describe("long-term lifecycle", () => {
  it("calculates natural usage days dynamically and freezes them after ending", () => {
    ...
    expect(timelineItemUsedDays(endTimelineItem(item, "2026-07-15"), "2026-08-01")).toBe(15);
  });

  it.each(["rigid_long_term", "lens_accessory", "opened_container", "batch_consumable"] as const)(
    "shows dynamic elapsed days for %s",
    (template) => {
      const itemWithStaleDisplayText = { ...item, detail: "已使用 1 天" };
      expect(timelineItemUsageSummary(itemWithStaleDisplayText, template, "2026-07-10")).toBe(
        "已使用 10 天"
      );
    }
  );
  ...
  it("excludes paused calendar days from active consumption days", () => {
    const resumed = resumeTimelineItem(pauseTimelineItem(item, "2026-07-04"), "2026-07-07");
    expect(timelineItemActiveDays(resumed, "2026-07-10")).toBe(7);
  });

  it("formats future, due-today, and overdue replacement countdowns", () => {
    expect(replacementCountdownText("2026-08-11", "2026-08-01")).toBe("预计 10 天后更换");
    expect(replacementCountdownText("2026-08-01", "2026-08-01")).toBe("预计今天更换");
    expect(replacementCountdownText("2026-07-30", "2026-08-01")).toBe("已超过预计更换日 2 天");
  });
  ...
  it("edits a pause and keeps adjacent intervals continuous", () => {
    const resumed = resumeTimelineItem(pauseTimelineItem(item, "2026-07-10"), "2026-07-13");
    const edited = editPausedInterval(resumed, 1, "2026-07-09", "2026-07-12");

    expect(edited.stateIntervals?.[0]?.endDate).toBe("2026-07-09");
    expect(edited.stateIntervals?.[2]?.startDate).toBe("2026-07-12");
  });

  it("deletes a historical pause and merges the adjacent active stages", () => {
    const resumed = resumeTimelineItem(pauseTimelineItem(item, "2026-07-10"), "2026-07-13");
    const deleted = deletePausedInterval(resumed, 1);

    expect(deleted.stateIntervals).toEqual([
      { startDate: "2026-07-01", endDate: null, status: "active" }
    ]);
  });

  it("deletes a final historical pause without changing a completed end date", () => {
    const completed = endTimelineItem(pauseTimelineItem(item, "2026-07-10"), "2026-07-15");
    const deleted = deletePausedInterval(completed, 1);

    expect(deleted.endDate).toBe("2026-07-16");
    expect(deleted.stateIntervals).toEqual([
      { startDate: "2026-07-01", endDate: "2026-07-16", status: "active" }
    ]);
  });

  it("rejects deleting the current pause or a non-pause stage", () => {
    const paused = pauseTimelineItem(item, "2026-07-10");
    expect(() => deletePausedInterval(paused, 1)).toThrow("当前暂停阶段不能删除，请先恢复使用");
    expect(() => deletePausedInterval(item, 0)).toThrow("暂停记录不存在");
    expect(() =>
      deletePausedInterval(
        {
          ...item,
          stateIntervals: [
            { startDate: "2026-07-01", endDate: "2026-07-05", status: "active" },
            { startDate: "2026-07-05", endDate: "2026-07-08", status: "paused" },
            { startDate: "2026-07-08", endDate: null, status: "paused" }
          ]
        },
        1
      )
    ).toThrow("暂停记录后的使用阶段不完整");
  });

  it("keeps a completed item's end boundary aligned with its final pause", () => {
    const completed = endTimelineItem(pauseTimelineItem(item, "2026-07-10"), "2026-07-15");
    const edited = editPausedInterval(completed, 1, "2026-07-09", "2026-07-18");
  ...
  it("rejects invalid pause, resume, end, and reopen transitions", () => {
    ...
    expect(() => resumeTimelineItem(item, "2026-07-02")).toThrow("只有暂停中的实例可以恢复");
    const paused = pauseTimelineItem(item, "2026-07-05");
    expect(() => resumeTimelineItem(paused, "2026-07-05")).toThrow("恢复日期必须晚于暂停日期");
    expect(() => endTimelineItem(item, "2026-06-30")).toThrow("结束日期不能早于启用日期");
    expect(() => reopenTimelineItem(item)).toThrow("只有已经结束的实例可以撤销结束");
  });

  it("rejects pause edits that create zero-length or open historical intervals", () => {
    const resumed = resumeTimelineItem(pauseTimelineItem(item, "2026-07-10"), "2026-07-13");
    expect(() => editPausedInterval(resumed, 1, "2026-07-01", "2026-07-12")).toThrow(
      "暂停日期必须晚于前一个使用阶段开始日期"
    );
    expect(() => editPausedInterval(resumed, 1, "2026-07-09", null)).toThrow(
      "历史暂停记录必须填写恢复日期"
    );
    expect(() => editPausedInterval(resumed, 1, "2026-07-09", "2026-07-09")).toThrow(
      "恢复日期必须晚于暂停日期"
    );
  });
  ...
  it("rejects malformed current intervals and duplicate lifecycle actions", () => {
    expect(() => pauseTimelineItem({ ...item, status: "paused" }, "2026-07-02")).toThrow(
      "只有使用中的实例可以暂停"
    );
    expect(() =>
      pauseTimelineItem(
      ...
      endTimelineItem(
        {
          ...item,
          locationIntervals: [
            { locationId: "home", startDate: "2026-07-01", endDate: "2026-07-02" }
          ]
        },
        "2026-07-03"
      )
    ).toThrow("地点末尾区间与结束日期冲突");
    expect(() =>
      endTimelineItem(
        {
          ...item,
          eyeAssignmentIntervals: [
            { eyeSides: ["L"], startDate: "2026-07-01", endDate: "2026-07-02" }
          ]
        },
        "2026-07-03"
      )
    ).toThrow("眼别末尾区间与结束日期冲突");
    const completed = endTimelineItem(item, "2026-07-03");
    expect(() =>
      reopenTimelineItem({
        ...completed,
  ...
  it("rejects malformed pause edits and later interval conflicts", () => {
    expect(() => editPausedInterval(item, 0, "2026-07-02", null)).toThrow("暂停记录不存在");
    const multiStage = {
      ...item,
      ...
      stateIntervals: [
        {
          ...
          endDate: "2026-07-05" as const,
          status: "active" as const
        },
        {
          startDate: "2026-07-05" as const,
          endDate: "2026-07-08" as const,
          status: "paused" as const
        },
        {
          startDate: "2026-07-08" as const,
          endDate: "2026-07-12" as const,
          status: "active" as const
        },
        {
          startDate: "2026-07-12" as const,
          endDate: "2026-07-20" as const,
          status: "paused" as const
        }
      ]
    };
    expect(() => editPausedInterval(multiStage, 1, "2026-07-04", null)).toThrow(
      "历史暂停记录必须填写恢复日期"
    );
    expect(() => editPausedInterval(multiStage, 1, "2026-07-04", "2026-07-12")).toThrow(
      "恢复日期必须早于后续阶段结束日期"
    );
    expect(() => editPausedInterval(multiStage, 1, "2026-07-04", "2026-07-21")).toThrow(
      "恢复日期必须早于后续阶段结束日期"
    );
    expect(() =>
      editPausedInterval(
