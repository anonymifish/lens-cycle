    ...
    lots: [
      {
        ...
        initialPackageQuantity: 2,
        initialLooseUnitQuantity: 0,
        unitsPerPackageAtReceipt: 1,
...
describe("populated page rendering", () => {
  it("opens a long-term rigid-lens record from a normal unpackaged lot", () => {
    useInventoryStore.setState((state) => {
      const lots = state.lots.map((source) => {
        const {
          initialPackageQuantity,
          initialLooseUnitQuantity,
  ...
  it("rejects duplicate lots and excessive transfers before persistence", async () => {
    ...
    fireEvent.submit(form);
    expect(
      within(form).getByText("该批次已经使用或损耗 1 片，入库数量不能低于这个数")
    ).toBeTruthy();
  });
  ...
  it("keeps inventory dialogs and data stable when repository writes fail", async () => {
    const extraProduct: Product = {
      ...
      id: "unused-failure-product",
      brand: "失败测试产品",
      model: "F1",
      sortOrder: 1
    };
    useInventoryStore.setState((state) => ({
      products: [{ ...state.products[0]!, sortOrder: 0 }, extraProduct]
    }));
    ...
    setPersistenceCommandAdapterForTests(
      async <T,>() => Promise.reject("non-error write failure") as Promise<T>
    );
    render(<InventoryPage />);
  ...
  it("reorders and deletes an unused product and voids an unused stock lot", async () => {
            ...
            transactions: [
              {
                id: reversalId,
                stockLotId: lot.id,
                occurredDate: args?.voidedAt,
                type: "reverse",
                quantityDelta: -3,
                reversedTransactionId: "stock-in-2"
              }
            ]
          } as T;
        }
    ...
    await waitFor(() =>
      expect(
        useInventoryStore.getState().products.find((entry) => entry.id === "unused-product")
          ?.sortOrder
      ).toBe(0)
    );
    ...
    await waitFor(() =>
      expect(
        useInventoryStore.getState().products.some((entry) => entry.id === "unused-product")
      ).toBe(false)
    );
    ...
    await waitFor(() =>
      expect(
        useInventoryStore.getState().lots.find((entry) => entry.id === "lot-2")?.voidedAt
      ).toBe("2026-09-06")
    );
  ...
  it("creates and edits packaged soft-lens products and stock snapshots", async () => {
    ...
    fireEvent.change(within(stockForm).getByLabelText("生产日期"), {
      target: { value: "2026-09-05" }
    });
    fireEvent.change(within(stockForm).getByLabelText("预计到期日期"), {
      target: { value: "2026-09-04" }
    });
    fireEvent.submit(stockForm);
    expect(within(stockForm).getByText("预计到期日期不能早于生产日期")).toBeTruthy();
    fireEvent.change(within(stockForm).getByLabelText("预计到期日期"), {
      target: { value: "2027-09-05" }
    });
    ...
    await waitFor(() =>
      expect(useInventoryStore.getState().lots[0]).toMatchObject({
        initialUnitQuantity: 11,
        initialPackageQuantity: 2,
        initialLooseUnitQuantity: 1,
        unitsPerPackageAtReceipt: 5
      })
    );
    ...
    fireEvent.change(within(lotForm).getByLabelText("生产日期"), {
      target: { value: "2026-09-05" }
    });
    fireEvent.change(within(lotForm).getByLabelText("预计到期日期"), {
      target: { value: "2026-09-04" }
    });
    ...
    act(() =>
      useInventoryStore.setState((state) => ({
        transactions: [
          ...
          {
            id: "opened-package",
            stockLotId: editedLot.id,
            occurredDate: "2026-09-06",
            type: "package_open",
            quantityDelta: 0,
            allocatedUnitQuantity: 5,
    ...
    fireEvent.submit(editForm);
    expect(
      within(editForm).getByText("该批次已经分配 1 片散片，入库散片数不能低于这个数")
    ).toBeTruthy();
  ...
  it("renders product fields for every inventory management template", () => {
    ...
    fireEvent.change(within(form).getByLabelText(/^用品配置/), {
      target: { value: "inventory-accessory" }
    });
    expect(within(form).getByLabelText("默认更换周期（天）")).toBeTruthy();
  ...
  it("renders statistics onboarding, warning severities, costs, and chart hover", () => {
    ...
    const warningLots: StockLot[] = expiryDates.map((expiryDate, index) => ({
      id: `warning-lot-${index}`,
      productId: baseProduct.id,
      internalLotCode: `WARNING-${index}`,
    ...
    useInventoryStore.setState({
      products: [baseProduct, lowProduct, emptyProduct],
      locations: [],
      lots: warningLots,
      transactions: [
        ...warningTransactions,
  ...
  it("renders opened-container and batch forecasts and groups minor pie categories", () => {
    ...
    const profiles: ItemProfile[] = profileSpecs.map(
      ([id, managementTemplate, standardType], index) => ({
        id: `stats-profile-${id}`,
        groupId: managementTemplate === "lens_accessory" ? "periodic" : "consumables",
        managementTemplate,
        standardType,
        standardTypeName: `统计类别 ${index + 1}`,
        name: `统计配置 ${index + 1}`,
        baseUnit: index === 0 ? "瓶" : "份",
        defaultDurationDays: 30,
        active: true,
        order: index
      })
    ...
    const lots: StockLot[] = products.map((product, index) => ({
      ...
      internalLotCode: `STATS-${index}`,
      receivedDate: "2026-08-01",
      locationId: "home",
      initialUnitQuantity: 3,
      unitPriceMinor: 1000 - index * 50,
      currency: "CNY"
    }));
    const transactions: InventoryTransaction[] = lots.map((lot, index) => ({
      id: `stats-stock-${index}`,
      stockLotId: lot.id,
      occurredDate: "2026-08-01",
      type: "stock_in",
      quantityDelta: 3,
    ...
    const items: TimelineItem[] = [
      {
        id: "stats-opened",
        categoryId: profiles[0]!.id,
        groupId: "consumables",
        categoryName: profiles[0]!.name,
        productId: products[0]!.id,
        sourceStockLotId: lots[0]!.id,
        label: "护理液使用",
        detail: "",
        location: "家",
        locationId: "home",
      ...
      {
        id: "stats-batch",
        categoryId: profiles[1]!.id,
        groupId: "consumables",
        categoryName: profiles[1]!.name,
        productId: products[1]!.id,
        sourceStockLotId: lots[1]!.id,
        label: "盐水使用",
        detail: "",
        location: "家",
        locationId: "home",
      ...
      items: [
        {
          ...
          sourceStockLotId: "missing-lot",
          label: "缺失关联实例",
          detail: "",
          location: "旧地点",
          locationId: "missing-location",
          locationIntervals: [
            { locationId: "missing-location", startDate: "2026-01-01", endDate: null }
          ],
      ...
      items: [
        {
          ...
          locationIntervals: [
            { locationId: "home", startDate: base.startDate, endDate: "2026-09-02" }
          ],
          eyeAssignmentIntervals: [
            { startDate: base.startDate, endDate: "2026-09-02", eyeSides: ["L"] }
          ]
        }
      ]
    });
  ...
  it("shows and edits lifecycle history for a non-rigid paused item", async () => {
    ...
    expect(useTimelineItemStore.getState().items[0]?.stateIntervals?.[1]?.startDate).toBe(
      "2026-08-20"
    );
    ...
    const refreshedHistory = within(screen.getByLabelText("用品详情"))
      .getByText("使用历史")
      .closest("section")!;
    fireEvent.click(within(refreshedHistory).getByRole("button", { name: "删除阶段" }));
    await waitFor(() =>
      expect(useTimelineItemStore.getState().items[0]?.stateIntervals).toEqual([
        { startDate: "2026-01-02", endDate: "2026-09-01", status: "active" },
        { startDate: "2026-09-01", endDate: null, status: "paused" }
      ])
    );
  });
  ...
  it("moves all remaining discrete-dose stock with a timeline location change", async () => {
    ...
    useInventoryStore.setState({
      ...
      lots: [
        {
          id: "lot-dose",
          productId: "product-dose",
          internalLotCode: "DOSE-1",
          receivedDate: "2026-08-01",
          locationId: "home",
        ...
        },
        {
          id: "lot-dose-other",
          productId: "product-dose",
          internalLotCode: "DOSE-2",
          receivedDate: "2026-08-02",
          locationId: "home",
          initialUnitQuantity: 4,
          unitPriceMinor: 100,
          currency: "CNY"
        }
      ],
    ...
    useTimelineItemStore.setState({
      items: [
        {
          id: "item-dose",
          categoryId: "profile-dose",
          groupId: "consumables",
          categoryName: "除蛋白液",
          productId: "product-dose",
          sourceStockLotId: "lot-dose",
          label: "除蛋白液批次",
          detail: "使用中",
          location: "家",
          locationId: "home",
    ...
    expect(
      useInventoryStore
        .getState()
        .transactions.filter((transaction) => transaction.stockLotId === "lot-dose-other")
    ).toHaveLength(1);
    ...
    expect(
      useInventoryStore
        .getState()
        .transactions.find((transaction) => transaction.id === "consume-dose")?.locationId
    ).toBe("home");
    drawer = screen.getByLabelText("用品详情");
  ...
  it("reconciles batch stock from an explicit end count and labels actual inventory", async () => {
    ...
    useTimelineItemStore.setState({
      items: [
        {
          id: "item-batch",
          categoryId: "profile-batch",
          groupId: "consumables",
          categoryName: "擦手纸",
          productId: "product-batch",
          sourceStockLotId: "lot-batch",
          label: "擦手纸批次",
          detail: "使用中",
          location: "家",
          locationId: "home",
    ...
    expect((within(form).getByLabelText(/^实际剩余数量/) as HTMLInputElement).value).toBe("");
    fireEvent.submit(form);
    ...
    expect(container.querySelector('g[role="button"] > rect')?.getAttribute("width")).toBe(
      activeBarWidth
    );
    expect(useInventoryStore.getState().transactions.at(-1)).toMatchObject({
      stockLotId: "lot-batch",
  ...
  it("operates timeline search, filters, scale, zoom, pan, and group controls", () => {
    ...
    expect(useTimelineViewportStore.getState().viewport.pixelsPerDay).not.toBe(
      initial.pixelsPerDay
    );
    ...
    useInventoryStore.setState((state) => ({
      ...
      lots: [
        ...
        },
        {
          id: "lot-4",
          productId: "product-2",
          internalLotCode: "20260401-1",
          expectedUsageDays: 30,
  ...
  it("commits rigid-lens lifecycle, detail, location, and care-event changes", async () => {
    ...
    fireEvent.click(within(drawer).getByRole("button", { name: "保存资料" }));
    await waitFor(() =>
      expect(useTimelineItemStore.getState().items[0]?.label).toBe("更新后的硬镜")
    );
    ...
    await waitFor(() =>
      expect(
        useTimelineItemStore
          .getState()
          .items[0]?.stateIntervals?.find((interval) => interval.status === "paused")?.startDate
      ).toBe("2026-09-03")
    );
    ...
    await waitFor(() =>
      expect(
        useTimelineCareEventStore
          .getState()
          .events.some((event) => event.kind === "protein" && event.completedDate === "2026-09-05")
      ).toBe(true)
    );
    ...
    fireEvent.click(
      screen
        .getAllByRole("button", { name: "确认完成" })
        .find((button) => button.getAttribute("type") === "submit")!
    );
    await waitFor(() =>
      expect(useTimelineCareEventStore.getState().events[0]?.completedDate).toBe("2026-09-06")
    );
  ...
  it("deletes an incorrectly activated timeline instance and restores its inventory", async () => {
    ...
    fireEvent.click(container.querySelector('g[role="button"]')!);
    fireEvent.click(
      within(screen.getByLabelText("用品详情")).getByRole("button", { name: "删除误录记录" })
    );
    ...
    expect(
      useInventoryStore
        .getState()
        .transactions.some((entry) => entry.reversedTransactionId === "activate-1")
    ).toBe(true);
    expect(
      useInventoryStore
        .getState()
        .transactions.find((entry) => entry.reversedTransactionId === "activate-1")
  ...
  it("rejects invalid lifecycle and care-event dates without persisting", async () => {
    ...
    fireEvent.change(within(form).getByLabelText("实际结束日期"), {
    ...
    fireEvent.change(within(form).getByLabelText("实际完成日期"), {
      target: { value: "2025-12-31" }
    });
    fireEvent.submit(form);
  ...
  it("rejects care-event plans and completions after an item has ended", () => {
    useTimelineItemStore.setState((state) => ({
      items: state.items.map((item) => ({
        ...
        stateIntervals: [
          { startDate: item.startDate, endDate: "2026-09-05", status: "active" as const }
        ]
      }))
    }));
    useTimelineCareEventStore.setState({
    ...
    fireEvent.submit(form);
    expect(
      within(form).getByText("已经结束使用的镜片不能新增护理计划，可补录已完成事件")
    ).toBeTruthy();
        ...
        items: state.items.map((item) =>
          item.id === "item-1"
            ? {
                ...item,
                stateIntervals: [
                  { startDate: item.startDate, endDate: "2026-02-15", status: "active" }
                ]
              }
            : item
        )
  ...
  it("renders every management template and exposes its lifecycle actions", async () => {
    ...
    const lots: StockLot[] = templates.map(([id], order) => ({
      id: `lot-${id}`,
      productId: `product-${id}`,
      internalLotCode: `202608-${order + 1}`,
      receivedDate: "2026-08-01",
      locationId: "home",
    ...
    const transactions: InventoryTransaction[] = templates.flatMap(([id]) => [
      ...
      ...(id === "reusable"
        ? [
            {
              id: "transfer-reusable",
              stockLotId: "lot-reusable",
              occurredDate: "2026-09-01",
    ...
    const items: TimelineItem[] = templates.map(([id, groupId, managementTemplate, , name]) => ({
      ...
      label: `${name}实例`,
      detail: "覆盖测试",
      location: id === "reusable" ? "随身" : "家",
      locationId: id === "reusable" ? "carry" : "home",
      locationIntervals:
        id === "reusable"
          ? [
              { locationId: "home", startDate: "2026-08-10", endDate: "2026-09-01" },
              { locationId: "carry", startDate: "2026-09-01", endDate: null }
            ]
          : [{ locationId: "home", startDate: "2026-08-10", endDate: null }],
      startDate: "2026-08-10",
      endDate: null,
      predictionDate: managementTemplate === "opened_container" ? "2026-09-20" : "2026-09-09",
      ...(managementTemplate === "opened_container"
        ? {
            openedExpiryDate: "2026-10-01",
            depletionPredictionDate: "2026-09-20",
            capacityMl: 120,
            usageRatePerDay: 2
          }
        : {}),
      ...
      ...(managementTemplate === "soft_daily"
        ? {
            ...
            inventorySourceKind: "package" as const
          }
        : {}),
      status: "active",
      stateIntervals: [{ startDate: "2026-08-10", endDate: null, status: "active" }]
    ...
    useInventoryStore.setState({
      ...
      locations: [
        { id: "home", name: "家", active: true, order: 0 },
        { id: "carry", name: "随身", active: true, order: 1 }
      ]
    });
    useTimelineItemStore.setState({ items });
    useTimelineCareEventStore.setState({ events: [] });
    ...
    for (const [, , managementTemplate, , name] of templates) {
      ...
      if (managementTemplate === "soft_daily" || managementTemplate === "soft_reusable") {
        ...
        if (managementTemplate === "soft_daily") {
          ...
          expect(
            within(addForm).getByText("只分配入库时登记的散片，不会占用已开封盒的剩余片数")
          ).toBeTruthy();
        } else {
    ...
    fireEvent.submit(usageForm);
    expect(
      within(usageForm).getByText("这一天已经记录过正常佩戴，同日第二片请记为额外消耗")
    ).toBeTruthy();
    ...
    fireEvent.click(screen.getByRole("button", { name: "保存" }));
    await waitFor(() =>
      expect(useUsageFactStore.getState().facts.some((fact) => fact.reason === "掉落")).toBe(true)
    );
    ...
    fireEvent.click(within(wearRecord).getByRole("button", { name: "撤销" }));
    await waitFor(() =>
      expect(within(screen.getByLabelText("用品详情")).queryByText("正常佩戴")).toBeNull()
    );
    ...
    const discardFactId = useTimelineItemStore
      .getState()
      .items.find((item) => item.id === "item-daily")?.completionUsageFactId;
    drawer = screen.getByLabelText("用品详情");
    ...
    expect(useUsageFactStore.getState().facts.some((fact) => fact.id === discardFactId)).toBe(
      false
    );
    fireEvent.click(
      within(screen.getByLabelText("用品详情")).getByRole("button", { name: "关闭详情" })
    );
    ...
    fireEvent.click(screen.getByRole("button", { name: "确认记录" }));
    await waitFor(() =>
      expect(useUsageFactStore.getState().facts.some((fact) => fact.kind === "dose")).toBe(true)
    );
    fireEvent.click(
      within(screen.getByLabelText("用品详情")).getByRole("button", { name: "关闭详情" })
    );
    ...
    fireEvent.change(within(assignmentForm).getByLabelText("切换日期"), {
      target: { value: "2026-08-09" }
    });
    fireEvent.submit(assignmentForm);
    expect(
      within(assignmentForm).getByText("切换日期必须在当前眼别阶段开始日期和今天之间")
    ).toBeTruthy();
    fireEvent.change(within(assignmentForm).getByLabelText("切换日期"), {
    ...
    expect(
      useTimelineItemStore.getState().items.find((item) => item.id === "item-case")?.eyeSides
    ).toEqual(["L"]);
    persistenceFailure = null;
    ...
    fireEvent.submit(eyeForm);
    expect(
      within(eyeForm).getByText("当前使用阶段不填写结束日期，请使用切换眼别创建下一段")
    ).toBeTruthy();
    ...
    expect(useInventoryStore.getState().transactions.at(-1)).toMatchObject({
      type: "activate",
      locationId: "carry",
      relatedInstanceId: "item-reusable"
    });
    ...
    await waitFor(() =>
      expect(
        useTimelineItemStore.getState().items.find((item) => item.id === "item-accessory")?.status
      ).toBe("completed")
    );
    fireEvent.click(
      within(screen.getByLabelText("用品详情")).getByRole("button", { name: "关闭详情" })
    ...
    for (const [index, id] of [
      ...
      await waitFor(() =>
        expect(
          useTimelineItemStore.getState().items.find((item) => item.id === `item-${id}`)?.status
        ).toBe("paused")
      );
    ...
    for (const index of [1, 2, 3, 5]) {
      ...
      await waitFor(() =>
        expect(useTimelineItemStore.getState().items).toHaveLength(previousCount + 1)
      );
    }
  }, 50_000);
  ...
  it("validates reusable-lens history edits", async () => {
    ...
    useInventoryStore.setState({
      products: [
        {
          id: "product-reusable",
          itemProfileId: "profile-reusable",
          standardType: "soft_reusable",
          brand: "测试复用镜",
          baseUnit: "片",
          unitsPerPackage: 6,
    ...
    useTimelineItemStore.setState({
      items: [
        {
          id: "item-reusable",
          categoryId: "profile-reusable",
          groupId: "lenses",
          categoryName: "复用软镜（L）",
          productId: "product-reusable",
          sourceStockLotId: "lot-reusable",
          label: "复用软镜盒",
          detail: "",
          location: "家",
          locationId: "home",
    ...
    const reopenedForm = screen.getByRole("heading", { name: "编辑镜片信息" }).closest("form")!;
    ...
    await waitFor(() =>
      expect(useTimelineItemStore.getState().items[0]?.startDate).toBe("2026-08-02")
    );
    expect(useTimelineItemStore.getState().items[0]?.stateIntervals?.[0]?.startDate).toBe(
      "2026-08-02"
    );
    expect(useTimelineItemStore.getState().items[0]?.locationIntervals?.[0]?.startDate).toBe(
      "2026-08-02"
    );
    expect(useTimelineItemStore.getState().items[0]?.eyeAssignmentIntervals?.[0]?.startDate).toBe(
      "2026-08-02"
    );
  });
  ...
  it("creates and manages configurable profile rows", async () => {
    ...
    fireEvent.change(screen.getByLabelText("自定义标准类型名称"), {
      target: { value: "测试软镜" }
    });
    ...
    await waitFor(() =>
      expect(
        useItemProfileStore.getState().profiles.find((profile) => profile.name === "已编辑右镜")
          ?.order
      ).toBe(1)
    );
    row = screen.getByText("已编辑右镜").closest("article")!;
  ...
  it("protects related profile fields and reports configuration action failures", async () => {
    ...
    useTimelineItemStore.setState({ items: [] });
    setPersistenceCommandAdapterForTests(
      async <T,>() => Promise.reject("non-error failure") as Promise<T>
    );
